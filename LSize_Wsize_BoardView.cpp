...
BoardView::BoardView(int lengthsize, int widthsize, int pX, int pY) : Lsize(lengthsize), Wsize(widthsize), left(pX), top(pY)
{
	pokemons = new int*[Lsize];
	for (int i = 0; i < lengthsize; i++)
		pokemons[i] = new int[Wsize];
	pBoard = new Point * [lengthsize];
	for (int i = 0; i < lengthsize; i++)		
		pBoard[i] = new Point[widthsize];
	background = new string[lengthsize * 10];
}
...
int BoardView::getLengthSize()
{
	return Lsize;
}

int BoardView::getWidthSize()
{
	return Wsize;
}
...
void BoardView::showBoard()
{
	...
	// Draw right line
	for (int i = 1; i < Wsize * 4; i++)
	{
		Sleep(10);
		Controller::gotoXY(Lsize * 8 + left + 1, i + top);
		if (i % 4 != 0)
			putchar(186);
		else
			putchar(186);
	}
	Controller::gotoXY(Lsize * 8 + left + 1, Wsize * 4 + top);
	putchar(188);
	
	// Draw bottom line
	for (int i = 1; i < Lsize * 8; i++)
	{
		Controller::gotoXY(Lsize * 8 + left - i + 1, Wsize * 4 + top);
		Sleep(5);
		if (i % 8 == 0)
			putchar(205);
		else
	...
	Controller::gotoXY(left + 1, Wsize * 4 + top);
	putchar(200);
	...
	{
		Sleep(10);
		Controller::gotoXY(left + 1, Wsize * 4 + top - i);
		if (i % 4 != 0)
			putchar(186);
		else
			putchar(186);
...
void BoardView::buildBoardData() {
	...
	bool* checkDuplicate = new bool[Lsize * Wsize];
	int* pos = new int[Lsize * Wsize];
	...
	for (int i = 0; i < Lsize; i++) {
		for (int j = 0; j < Wsize; j++) {
			int r = pos[Lsize * i + j] / Wsize;
			int c = pos[Lsize * i + j] % Wsize;
			cout << r << " " << c << endl;
...
void BoardView::selectedBlock(int x, int y, int color) {
	Controller::setConsoleColor(LIGHT_BLUE, BRIGHT_WHITE);
...
void BoardView::deleteBlock(int x, int y)
{
	...
	if (y - 4 >= getYAt(0, 0) && getCheck(x, y - 4) == _DELETE) {
		for (int i = x - 3; i <= x + 3; i++) {
			...
			//putchar(32);
			putchar(background[y - 1 - top][i - left]);

		}
	}
	//Delete bottom border
	if (y + 4 <= getYAt(Lsize - 1, Wsize - 1) && getCheck(x, y + 4) == _DELETE) {
		for (int i = x - 3; i <= x + 3; i++) {
			Controller::gotoXY(i, y + 2);
			//putchar(32);
	...
	//Delete right border
	if (x + 8 <= getXAt(Lsize - 1, Wsize - 1) && getCheck(x + 8, y) == _DELETE) {
		for (int i = y - 1; i <= y + 1; i++) {
			Controller::gotoXY(x + 4, i);
			//putchar(32);
			putchar(background[i - top][x + 4 - left]);
